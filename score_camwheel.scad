 ...
 *   post to sit (not including the height of the support retainer) in mm
...
module score_camwheel(specification, optimize_for_fdm = false) {
  ...
  collar_boss_recess_depth = get_specification_argument(specification, score_camwheel_specification_collar_boss_recess_depth_parameter_name());
  collar_boss_recess_diameter = get_specification_argument(specification, score_camwheel_specification_collar_boss_recess_diameter_parameter_name());
  set_screw_hole_diameter = get_specification_argument(specification, score_camwheel_specification_set_screw_hole_diameter_parameter_name());
  set_screw_hole_offset = get_specification_argument(specification, score_camwheel_specification_set_screw_hole_offset_parameter_name());
  ...
  support_retainer_outer_diameter = get_specification_argument(specification, score_camwheel_specification_support_retainer_outer_diameter_parameter_name());
  support_retainer_height = get_specification_argument(specification, score_camwheel_specification_support_retainer_height_parameter_name());
  support_retainer_fillet_radius = get_specification_argument(specification, score_camwheel_specification_support_retainer_fillet_radius_parameter_name());
  support_recess_depth = get_specification_argument(specification, score_camwheel_specification_support_recess_depth_parameter_name());
  support_recess_diameter = get_specification_argument(specification, score_camwheel_specification_support_recess_diameter_parameter_name());
  index_hole_diameter = get_specification_argument(specification, score_camwheel_specification_index_hole_diameter_parameter_name());
  index_hole_spacing = get_specification_argument(specification, score_camwheel_specification_index_hole_spacing_parameter_name());
  index_hole_boss_diameter = get_specification_argument(specification, score_camwheel_specification_index_hole_boss_diameter_parameter_name());
  index_hole_boss_height = get_specification_argument(specification, score_camwheel_specification_index_hole_boss_height_parameter_name());
  index_hole_boss_fillet_radius = get_specification_argument(specification, score_camwheel_specification_index_hole_boss_fillet_radius_parameter_name());
  ...
  assert(index_hole_diameter > 0, "the camwheel index hole diameter must be greater than zero");
  assert(index_hole_spacing >= (index_hole_diameter + collar_outer_diameter) / 2, "the camwheel index hole spacing must be at least as long as the combined radiuses of the index hole and shaft collar");
  assert(index_hole_spacing <= wheel_minor_diameter - index_hole_diameter / 2, "the camwheel index hole spacing must be no more than the radius of the camwheel minor diameter minus the radius of the index hole");
  assert(index_hole_boss_diameter > index_hole_diameter, "the camwheel index hole boss diameter must be greater than the index hole diameter");
  assert(index_hole_boss_height > 0, "the camwheel index hole boss height must be greater than zero");
  assert(index_hole_boss_fillet_radius >= 0, "the camwheel index hole boss fillet radius must not be negative");
  ...
  index_position = polar_to_cartesian(theta = 0, radius = index_hole_spacing);
  ...
  difference() {
    union() {
      ...
      // Geometry on top of the camwheel
      ...
      // Geometry on the bottom of the camwheel
      if (!optimize_for_fdm) {
        translate([0, 0, epsilon()]) {
          rotate([180, 0, 0]) {
            // Index hole boss
            translate(index_position) {
              cylinder_with_fillet(
                  base_diameter = index_hole_boss_diameter,
                  top_diameter = index_hole_boss_diameter,
                  height = index_hole_boss_height,
                  fillet_radius = index_hole_boss_fillet_radius
              );
            }
    ...
    // Set screws
    for(screw_hole = [0:2]) {
      translate([0, 0, collar_height + wheel_thickness - set_screw_hole_offset]) {
        rotate([0, 90, screw_hole * (360 / supports) + (360 / supports) / 2]) {
          cylinder(d = set_screw_hole_diameter, h = collar_outer_diameter / 2 + epsilon());
        }
      }
    }
    ...
    // Index hole
    translate(index_position) {
      cylinder(d = index_hole_diameter, h = total_height * 2.5, center = true);
    }
    
    // Support post recesses
    for(support_position = support_positions) {
      translate([0, 0, support_recess_depth]) {
        translate(support_position) {
          rotate([180, 0, 0]) {
            cylinder(
                d = support_recess_diameter,
                h = (support_recess_depth + support_retainer_height) + epsilon()
            );
          }
        }
      }
    ...
    // Shaft collar boss recess
    translate([0, 0, collar_boss_recess_depth]) {
      rotate([180, 0, 0]) {
        cylinder(d = collar_boss_recess_diameter, h = collar_boss_recess_depth + epsilon());
      }
    }
